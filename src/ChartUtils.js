 ...
 * @param {Number} e 
 */
function chartInit(chart,gradient,{offset,scale},onUpdate,coordinateState,showDotted,setOpacity,gradientState,e){
  if(!chart)
    return;

  const canvas=chart.canvas;
  const ctx=canvas.getContext('2d');
  const e0=e||Math.max(scale.x,scale.y)/Math.min(chart.chartArea.width,chart.chartArea.height); 
  ...
  const updateArgs=(type,index,x_,y_)=>{
    return {
      ...
      gradientPoints:updateGradient(chart,gradient,{offset,scale},setOpacity,gradientState)
    };
  };
  ...
  canvas.onmousemove=canvas.onmouseenter=(ev)=>{
    ...
    if(!setOpacity)
      y=GradientPointHeight;
    coordinateState.setPos({x,y});
  };
  canvas.onresize=()=>e0=e||Math.max(scale.x,scale.y)/Math.min(chart.chartArea.width,chart.chartArea.height);
}

export {normalizedData,getOptions,chartInit};