...
function updateGradient(chart, Gradient, normalization, setOpacity, toUpdate) {
  const { getGradient, gradientPoints } = setGradient(
    Gradient,
    chart.data.labels,
  ...
  } else toUpdate.getGradient = getGradient;
  chart.update();
...
function setGradient(palette, pointsX, pointsY, { offset, scale }, setOpacity) {
  ...
  if (setOpacity) {
    ...
    for (let j = 0; j < palette.length; j++) {
      if (i < pointsX.length) {
        ...
        const t =
          cur.x - prev.x === 0 ? 0 : (pointOffset - prev.x) / (cur.x - prev.x);
      ...
      } else gradientPoints.push(palette[j]);
      while (i < pointsX.length) {
        ...
        const t =
          (pointOffset - palette[j].offset) /
          (palette[j + 1].offset - palette[j].offset);
        const color = colors[j].map(
          (val, idx) => val * (1 - t) + colors[j + 1][idx] * t
        );
        const cssColor =
          "rgba(" +
          color[0] +
  ...
  const getGradient = (ctx, chartArea) => {
    ...
    if (!gradient || width !== chartWidth || height !== chartHeight) {
      ...
      gradientPoints.forEach((colorPoint) =>
        gradient.addColorStop(1 - colorPoint.offset, colorPoint.color)
      );
    }
